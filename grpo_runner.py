...
class GRPORunner:
    def get_model(self, training_args: GRPOConfig, model_args: ModelConfig, model_name: str):
        ...
        # Используем стандартный Transformers вместо Unsloth
        return AutoModelForCausalLM.from_pretrained(
            model_name,
            torch_dtype=model_args.torch_dtype,  # Берем torch_dtype из ModelConfig
            attn_implementation=model_args.attn_implementation,  # Берем flash_attention_2 из ModelConfig
    ...
    def run(
        ...
        model = self.get_model(training_args, model_args, model_name_or_path)
