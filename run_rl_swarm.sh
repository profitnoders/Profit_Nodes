...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    yarn install
    PORT=3033 yarn dev > /dev/null 2>&1 & # Run in background and suppress output
    ...
    if open http://localhost:3033 2> /dev/null; then
        echo_green ">> Successfully opened http://localhost:3033 in your default browser."
    else
        echo ">> Failed to open http://localhost:3033. Please open it manually."
    fi
    ...
    while true; do
        STATUS=$(curl -s "http://localhost:3033/api/get-api-key-status?orgId=$ORG_ID")
