...
class DataLoaderShard(DataLoaderAdapter, DataLoaderStateMixin):
  ...
  def __iter__(self):
      ...
      current_batch = None  # Инициализируем заранее
  
      try:
          current_batch = next(dataloader_iter)
      except StopIteration:
          return  # Лучше использовать return вместо yield для корректного завершения
  
      batch_index = 0
      while True:
          try:
              if current_batch is None:
                  break  # Если current_batch не был инициализирован, прерываем цикл
