    ...
    def run(self) -> None:
        ...
        async def _run():
            ...
            while True:
                ...
                if not self._inner_pipe.poll(0.1):
                    await asyncio.sleep(0.05)
                    continue

                try:
                    method, args, kwargs = self._inner_pipe.recv()
                except (BlockingIOError, EOFError, OSError, ConnectionError, RuntimeError) as e:
                    logger.debug(f"Pipe recv error: {e}", exc_info=True)
