...
function install_node() {
    ...
    docker run -d --restart unless-stopped --network=host -v ~/.titanedge:/root/.titanedge nezha123/titan-edge
...
function restart_node() {
    echo -e "${CLR_INFO}Перезапуск контейнера Titan...${CLR_RESET}"
    CONTAINER_ID=$(docker ps -aq --filter ancestor=nezha123/titan-edge)
    if [[ -n "$CONTAINER_ID" ]]; then
        docker update --restart unless-stopped "$CONTAINER_ID"
        docker restart "$CONTAINER_ID"
        echo -e "${CLR_SUCCESS}✅ Контейнер перезапущен и установлен флаг автоперезапуска!${CLR_RESET}"
    else
        echo -e "${CLR_WARNING}⚠️ Контейнер не найден. Возможно, он не был создан.${CLR_RESET}"
    fi
}
